...
function playMoveSound() { triggerVibration(40); playSynthSound(200, 'sine', 0.1); }
function playCaptureSound() { triggerVibration([100, 50, 100]); playSynthSound(100, 'square', 0.15); setTimeout(() => playSynthSound(80, 'sawtooth', 0.2), 50); }
...
// ==========================================
// PART 2: History, Board Setup & Modal Logic
// ==========================================
...
async function loadGlobalAIDatabase() {
    try { const response = await fetch(`${VERCEL_API_URL}/get-strategies`); if (response.ok) { const data = await response.json(); aiDatabase.userWinningTraps = data.traps || []; } } catch (error) { console.log("Offline fallback active."); }
}
async function syncTrapToVercelDatabase(winningMove) {
    try { await fetch(`${VERCEL_API_URL}/save-strategy`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ move: winningMove }) }); } catch (error) { console.log("Local database bypass."); }
}
...
function switchMode(mode) { currentMode = mode; if (mode === "Vs-AI") { gameMetrics.currentStage = 1; gameMetrics.consecutiveUserLosses = 0; gameMetrics.adaptiveDifficultyScore = 50; } triggerReset(); }

function triggerReset() {
    selectedSquare = null; highlightedMoves = []; isPlayer1Turn = true; isGameOver = false; capturedWhite = []; capturedBlack = []; gameMetrics.matchMoveHistory = []; resetInitialSetup(); modalElement.classList.add('hidden');
    const historyFeed = document.getElementById('move-history-feed');
...
function generateAITeachingReport(userWon) {
    ...
    if (userWon) {
        gameMetrics.consecutiveUserLosses = 0; gameMetrics.currentStage = Math.min(gameMetrics.currentStage + 1, 3); gameMetrics.adaptiveDifficultyScore = Math.min(gameMetrics.adaptiveDifficultyScore + 25, 100);
        if (gameMetrics.matchMoveHistory.length > 0) { const lastWinningMove = gameMetrics.matchMoveHistory[gameMetrics.matchMoveHistory.length - 1]; aiDatabase.userWinningTraps.push(lastWinningMove); syncTrapToVercelDatabase(lastWinningMove.to); }
    ...
    if (gameMetrics.consecutiveUserLosses >= 2) { gameMetrics.currentStage = Math.max(gameMetrics.currentStage - 1, 1); gameMetrics.adaptiveDifficultyScore = Math.max(gameMetrics.adaptiveDifficultyScore - 20, 50); gameMetrics.consecutiveUserLosses = 0; advise += "🛡️ Engine difficulty dynamically scaled down for balance.\n"; } 
    else { advise += "⚡ Engine playing at tactical level. "; if (gameMetrics.userAggressionCount < 3) advise += "Develop Padati and Ashva pieces in the early phase."; else if (gameMetrics.userMistakes.includes("Raja_exposed")) advise += "Tactical Error: Raja was left vulnerable with open lines of attack."; else advise += "Coordinate your Ratha and Gaja forces to establish board dominance."; }
    return advise;
}
...
function showEndGameModal(title, description, icon, userWon) {
    ...
    document.getElementById('modalIcon').innerText = icon; modalElement.classList.remove('hidden'); gameMetrics.userMistakes = []; gameMetrics.userAggressionCount = 0;
}
...
function calculatePossibleMoves(row, col, piece) { const validDestinations = []; for (let r = 0; r < 8; r++) { for (let c = 0; c < 8; c++) { if (checkLegalMove(piece, row, col, r, c)) validDestinations.push(`${r}-${c}`); } } return validDestinations; }
...
// ==========================================
// PART 3: Gameplay Interaction & AI Logic
// ==========================================
function createBoard() {
    ...
    for (let row = 0; row < 8; row++) {
        for (let col = 0; col < 8; col++) {
            ...
            if (piece) { const labelColor = piece.isWhite ? 'text-amber-700' : 'text-red-700'; square.innerHTML = `<span class="text-2xl md:text-3xl filter drop-shadow-sm">${pieceSigns[piece.name]}</span><span class="text-[7px] ${labelColor} font-sans font-extrabold z-10 uppercase tracking-tighter absolute bottom-0.5">${piece.name}</span>`; }
...
async function handleSquareClick(row, col) {
    ...
    if (selectedSquare === null) {
        ...
        if (targetPiece) {
            if (targetPiece.isWhite) { capturedWhite.push(targetPiece.name); if (targetPiece.name === 'Raja') { isGameOver = true; createBoard(); showEndGameModal("DEFEAT", "The opponent has captured your Raja!", "💀", false); return; } } 
            else { capturedBlack.push(targetPiece.name); if (targetPiece.name === 'Raja') { isGameOver = true; createBoard(); showEndGameModal("VICTORY!", "You have conquered the opposing throne!", "👑", true); return; } }
        }
...
function evaluateBoardState() {
    ...
    for (const key in initialSetup) { const piece = initialSetup[key]; const [r] = key.split('-').map(Number); let weight = scores[piece.name]; if (piece.name === 'Padati') weight += piece.isWhite ? (7 - r) : r; if (piece.isWhite) totalVal -= weight; else totalVal += weight; }
    return totalVal;
}
...
function minimax(depth, isAiMaximizing) {
    ...
    if (isAiMaximizing) {
        ...
        for (const move of aiMoves) { const backup = initialSetup[move.to]; initialSetup[move.to] = initialSetup[move.from]; delete initialSetup[move.from]; let evaluation = minimax(depth - 1, false); maxEval = Math.max(maxEval, evaluation); initialSetup[move.from] = initialSetup[move.to]; if (backup) initialSetup[move.to] = backup; else delete initialSetup[move.to]; }
        return maxEval;
    } else {
        let minEval = Infinity;
        for (const move of aiMoves) { const backup = initialSetup[move.to]; initialSetup[move.to] = initialSetup[move.from]; delete initialSetup[move.from]; let evaluation = minimax(depth - 1, true); minEval = Math.min(minEval, evaluation); initialSetup[move.from] = initialSetup[move.to]; if (backup) initialSetup[move.to] = backup; else delete initialSetup[move.to]; }
        return minEval;
    }
}
...
function triggerAiEngineLogic() {
    ...
    for (const key in initialSetup) {
        if (!initialSetup[key].isWhite) {
            const [fromR, fromC] = key.split('-').map(Number), piece = initialSetup[key];
            for (let toR = 0; toR < 8; toR++) { for (let toC = 0; toC < 8; toC++) { if (checkLegalMove(piece, fromR, fromC, toR, toC)) { const target = initialSetup[`${toR}-${toC}`]; allLegalAiMoves.push({ fromKey: key, toKey: `${toR}-${toC}`, piece: piece, targetPiece: target }); } } }
        }
    }
    ...
    if (gameMetrics.currentStage === 1) { allLegalAiMoves.sort(() => Math.random() - 0.5); } 
        ...
        for (const move of allLegalAiMoves) { const backup = initialSetup[move.toKey]; initialSetup[move.toKey] = initialSetup[move.fromKey]; delete initialSetup[move.fromKey]; move.minimaxWeight = evaluateBoardState(); initialSetup[move.fromKey] = initialSetup[move.toKey]; if (backup) initialSetup[move.toKey] = backup; else delete initialSetup[move.toKey]; }
        allLegalAiMoves.sort((a, b) => b.minimaxWeight - a.minimaxWeight);
    } else {
        for (const move of allLegalAiMoves) { const backup = initialSetup[move.toKey]; initialSetup[move.toKey] = initialSetup[move.fromKey]; delete initialSetup[move.fromKey]; move.minimaxWeight = minimax(2, false); aiDatabase.userWinningTraps.forEach(trap => { if (trap.to === move.toKey) move.minimaxWeight += 250; }); initialSetup[move.fromKey] = initialSetup[move.toKey]; if (backup) initialSetup[move.toKey] = backup; else delete initialSetup[move.toKey]; }
        allLegalAiMoves.sort((a, b) => b.minimaxWeight - a.minimaxWeight);
    }
...
loadGlobalAIDatabase(); triggerReset();
